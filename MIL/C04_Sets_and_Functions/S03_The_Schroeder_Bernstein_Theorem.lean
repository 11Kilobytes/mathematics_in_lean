...
theorem sb_injective (hf : Injective f) : Injective (sbFun f g) := by
  ...
  · wlog x₁A : x₁ ∈ A generalizing x₁ x₂ hxeq xA
    ...
    have x₂A : x₂ ∈ A := by
      ...
      have x₂eq : x₂ = g (f x₁) :=
        calc x₂ = g (invFun g x₂) := (sb_right_inv _ _ x₂nA).symm
             _ = g (f x₁)         := congrArg g (hxeq.symm)
  ...
  push_neg at xA
  rcases xA with ⟨neg_x₁A, neg_x₂A⟩
  rw [if_neg neg_x₁A, if_neg neg_x₂A] at hxeq
  calc x₁ = g (invFun g x₁) := (sb_right_inv _ _ neg_x₁A).symm
       _ =  g (invFun g x₂) := congrArg g hxeq
       _ =  x₂              := sb_right_inv _ _ neg_x₂A
