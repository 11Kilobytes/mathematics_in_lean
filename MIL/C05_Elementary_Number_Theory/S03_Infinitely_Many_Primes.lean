...
theorem primes_infinite : ∀ n, ∃ p > n, Nat.Prime p := by
  ...
  have : p ∣ Nat.factorial (n + 1) := by
    ...
    . linarith
  have : p ∣ 1 := by
    have : 1 = (Nat.factorial (n + 1) + 1) - (Nat.factorial (n + 1))
      := by simp
    rw [this]
...
theorem _root_.Nat.Prime.eq_of_dvd_of_prime {p q : ℕ}
    ...
    p = q := by
  rcases prime_q.eq_one_or_self_of_dvd p h with peq1 | h
  . rw [peq1] at prime_p
    contradiction
  . exact h
...
theorem mem_of_dvd_prod_primes {s : Finset ℕ} {p : ℕ} (prime_p : p.Prime) :
  ...
  rcases h₁ with h | h
  . left
    exact prime_p.eq_of_dvd_of_prime ap h
  . right
    exact ih sp h
...
theorem exists_prime_factor_mod_4_eq_3 {n : Nat} (h : n % 4 = 3) :
  ...
  . by_cases mp : m.Prime
    . use m
    . rcases ih m mltn h1 mp with ⟨p, pp, pdvdm, p_mod_4_eq_3⟩
      use p; exact ⟨pp, pdvdm.trans mdvdn, p_mod_4_eq_3⟩
  . rcases aux mdvdn mge2 mltn with ⟨n_div_m_dvd_n, n_div_m_lt_n⟩
    by_cases n_div_m_p : (n / m).Prime
    . use n / m
    . rcases ih (n / m) n_div_m_lt_n h1 n_div_m_p
        with ⟨p, pp, pdvd_n_div_m, p_mod_4_eq_3⟩
      use p; exact ⟨pp, pdvd_n_div_m.trans n_div_m_dvd_n, p_mod_4_eq_3⟩
...
theorem primes_mod_4_eq_3_infinite : ∀ n, ∃ p > n, Nat.Prime p ∧ p % 4 = 3 := by
  ...
  have ps : p ∈ s := by
    apply (hs p).mp; constructor <;> assumption
  have pne3 : p ≠ 3 := by
    intro h
    rw [h] at pdvd
    simp at pdvd
    have : 3 ∈ erase s 3 := by
      rcases Nat.prime_three.dvd_mul.mp pdvd with h | h
      . contradiction
    ...
    rcases this with ⟨habsurd, _⟩
    contradiction
  have : p ∣ 4 * ∏ i in erase s 3, i := by
    apply dvd_mul_of_dvd_right _ 4
    apply Finset.dvd_prod_of_mem
    rw [mem_erase]; constructor <;> assumption
  have : p ∣ 3 := by
    have : p ∣ ((4 * ∏ i in erase s 3, i + 3) - (4 * ∏ i in erase s 3, i)) :=
      by apply Nat.dvd_sub' <;> assumption
    convert this
    simp
  have : p = 3 := by
