...
example {m n : ℕ} (coprime_mn : m.Coprime n) : m ^ 2 ≠ 2 * n ^ 2 := by
  ...
  have : 2 * (2 * k ^ 2) = 2 * n ^ 2 := by
    ...
    ring
  have : 2 * k ^ 2 = n ^ 2 := by
    rcases mul_eq_mul_left_iff.mp this with h | absurd
    . exact h
    . contradiction
  have : 2 ∣ n := by
...
example {m n p : ℕ} (coprime_mn : m.Coprime n) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have : p * k ^ 2 = n ^ 2 := by
    rcases mul_eq_mul_left_iff.mp this with h | absurd
    . exact h
    . have : p ≥ 2 := (Nat.prime_def_lt.mp prime_p).1
      linarith
  have : p ∣ n := by
    ...
    apply dvd_mul_right
  have : p ∣ m.gcd n := by
    apply (dvd_gcd_iff p m n).mpr; constructor <;> assumption
...
example {m n p : ℕ} (nnz : n ≠ 0) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have eq2 : (p * n ^ 2).factorization p = 2 * n.factorization p + 1 := by
    calc (p * n ^ 2).factorization p
         _ = (p.factorization p) + (n ^ 2).factorization p
             := factorization_mul' prime_p.ne_zero nsqr_nez _
         _ = 1 + (n ^ 2).factorization p
             := by rw [prime_p.factorization_self]
         _ = 1 + 2 * (n.factorization p)
             := by rw [factorization_pow']
         _ = 2 * (n.factorization p) + 1
             := by rw [add_comm]
...
example {m n k r : ℕ} (nnz : n ≠ 0) (pow_eq : m ^ k = r * n ^ k) {p : ℕ} :
  ...
  have eq2 : ((r + 1) * n ^ k).factorization p =
    ...
    rw [factorization_mul' (by norm_num) npow_nz, factorization_pow', add_comm]
